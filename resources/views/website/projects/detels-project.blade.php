...
@section('title', $project->title ?? '' . ' - ' . $project->service->title ?? '' . ' |  Abarce Production')
@section('meta_description', Str::limit(strip_tags($project->description), 160))
@section('meta_tags')
    <meta property="og:title" content="{{ $project->title ?? ''}} - {{ $project->service->title ?? ''}}">
    <meta property="og:description" content="{{ Str::limit(strip_tags($project->description), 160) }}">
    <meta property="og:image" content="{{ asset('storage/' . ($project->images->first())?->image ?? 'default.jpg') }}">
    ...
    <meta name="twitter:image" content="{{ asset('storage/' . ($project->images->first())?->image ?? 'default.jpg') }}">
@endsection
...
@section('content')
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <a href="{{ route('landingpage') }}#portfolio" class="back-btn d-inline-flex align-items-center mb-4"
                    data-aos="fade-right">
                        ...
                        @foreach ($project->images as $imag)
                            <div class="col-md-4 col-sm-6 col-12">
                                <div class="gallery-item position-relative overflow-hidden rounded shadow-sm">
                                    <img src="{{ asset('storage/' . $imag->image) }}" alt="{{ $project->title }}"
                                        class="gallery-img img-fluid w-100" />
                                    <div class="gallery-overlay d-flex align-items-center justify-content-center">
                                        <i class="bi bi-zoom-in gallery-icon" data-bs-toggle="modal"
